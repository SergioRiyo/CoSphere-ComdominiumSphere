...
required_variables='RLS_DB_HOST RLS_DB_PORT RLS_DB_DATABASE RLS_DB_USERNAME RLS_DB_PASSWORD'
...
case "$RLS_DB_USERNAME" in
    cosphere_rls_admin|cosphere_app_test) ;;
    *)
        echo "RLS POC blocked: RLS_DB_USERNAME is not an RLS POC account." >&2
        exit 1
        ;;
esac
